 ...
 */                
  ...
  glPopMatrix();
  if (_REPERE_) glCallList(id_repere);
...
extern void g3x_InitWindow(char* windname, int w, int h)
{
  ...
  g3x_SetLightSpheric(0.25*PI,+0.25*PI,10.,(G3Xpoint){0.,0.,0.});
}
...
{ 
  ...
  glNewList(id_repere, GL_COMPILE);
    glPushMatrix();
      g3x_Arrow(0.025,1.0,(G3Xcolor){0.,0.,1.,0.66});
        glPushMatrix();
        glRotatef(-90.,1.,0.,0.);
        g3x_Arrow(0.025,1.0,(G3Xcolor){0.,1.,0.,0.66});
      glPopMatrix();  
      glPushMatrix();
        glRotatef(+90.,0.,1.,0.);
        g3x_Arrow(0.025,1.0,(G3Xcolor){1.,0.,0.,0.66});
      glPopMatrix();  
    glPopMatrix();  
  glEndList();
